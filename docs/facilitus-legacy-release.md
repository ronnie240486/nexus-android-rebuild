...
| Item | Valor |
|---|---|
| Arquivo | `dist/facilitus-legacy-0.2.12.apk` |
...
| DEX | Quatro DEX do APK original + `classes5.dex` da tela Facilitus |
...
A build foi alinhada, assinada com certificado próprio do Facilitus e verificada com AAPT2 e `apksigner`. Não foi possível executar instalação em um dispositivo real no sandbox porque não havia aparelho conectado via ADB; a validação disponível é estática e de integridade do APK.
